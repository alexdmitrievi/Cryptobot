...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "show_wallet":
        ...
        await query.edit_message_text(
            ...
            "📌 Перед тем как прислать скрин, убедись что:\n"
            "• В TradingView включён индикатор LazyScalp Board — он покажет DV.\n"
            "• DV на скрине больше 200M. Если DV < 200M, сетап строить запрещено.\n"
            "• Также желательно наложить Smart Money Concepts (LuxAlgo/WeloTrades) для BOS, ликвидности и OTE.\n\n"
            "🖼 Когда готово — пришли скрин, я построю сетап с планом входа, стопом и тейками.",
            parse_mode="Markdown"
        )
        ...
        await query.edit_message_text(
            ...
            "📌 Перед скрином включи LazyScalp Board в TradingView, чтобы видеть DV.\n"
            "Если DV < 200M — сетап строить нельзя.\n"
            "Рекомендуются уровни и объём (Volume Profile / Fixed Range).\n\n"
            "🖼 Пришли скрин — дам сценарии продолжения и разворота с пояснением.",
            parse_mode="Markdown"
        )
        ...
        await query.edit_message_text(
            "📈 *Пробой диапазона (Breakout)*\n\n"
            "📌 Убедись что на графике TradingView включён LazyScalp Board для DV.\n"
            "Если DV < 200M — сетап строить нельзя.\n"
            "Также желательно, чтобы были видны зоны флэта и объёмные индикаторы (Volume, OBV).\n\n"
            "🖼 Пришли скрин — я найду консолидацию и дам два сценария пробоя.",
            parse_mode="Markdown"
        )
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if selected_style == "smc":
        prompt_text = (
            "Ты — трейдер Smart Money Concepts с 10+ лет на крипте, форексе и MOEX.\n\n"
            "На скрине должен быть включён индикатор LazyScalp Board в TradingView — он показывает дневной объём (DV).\n"
            "❗️Если DV < 200M, напиши крупно 🚫, что сетап строить запрещено, ликвидности недостаточно.\n"
            "Если DV ≥ 200M:\n"
            "- Найди ликвидность (стопы, экстремумы)\n"
            "- BOS, CHoCH, OTE\n"
            "- Дай чёткий план Entry, StopLoss, TakeProfit\n"
            "Объясни почему именно здесь вход и что делает толпа."
        )
    elif selected_style == "swing":
        prompt_text = (
            "Ты — позиционный трейдер (swing) с опытом на крипте, форексе и MOEX.\n\n"
            "На скрине должен быть включён индикатор LazyScalp Board для DV.\n"
            "❗️Если DV < 200M, скажи крупно 🚫, что сетап строить нельзя, ликвидность мала.\n"
            "Если DV ≥ 200M:\n"
            "- Найди ключевые уровни поддержки/сопротивления\n"
            "- Зоны накоплений маркетмейкеров\n"
            "- Дай Entry, StopLoss, TakeProfit\n"
            "И укажи что ещё проверить (корреляцию, отчёты китов)."
        )
        ...
        prompt_text = (
            ...
            "❗️Если DV < 200M, напиши 🚫, что сетап не строим.\n"
            "Если DV ≥ 200M:\n"
            "- Найди диапазон консолидации (range)\n"
            "- Дай два сценария: breakout up и breakout down\n"
            "- Для каждого Entry, StopLoss, TakeProfit\n"
            "И что дополнительно проверить (дельта, кластера, стакан)."
        )
    else:
        prompt_text = (
            "Ты — трейдер с 10+ лет. На скрине TradingView включён LazyScalp Board.\n"
            "❗️Если DV < 200M, скажи 🚫, что сетап не строим.\n"
            "Если DV ≥ 200M — определи тренд, уровни, Entry, StopLoss, TakeProfit."
        )
...
IS_TEST = False  # False для боевого режима
