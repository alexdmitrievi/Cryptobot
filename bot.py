...
async def gpt_psychologist_response(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    prompt = (
        ...
        "Общайся легко, с доброй иронией, ставь эмодзи, не бойся подколоть — но всегда поддерживай и подбадривай.\n\n"
        f"Сообщение от пользователя:\n{user_text}\n\n"
        "Добавь в конце подходящий текстовый мем (не картинку), связанный с трейдингом. "
        "Пример мема: '— Ты держишь позицию? — Нет, я держу слёзы 😭'"
    )
    response = await client.chat.completions.create(
        model="gpt-4",
        messages=[{"role": "user", "content": prompt}]
