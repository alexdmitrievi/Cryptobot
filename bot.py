...
async def general_response(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    prompt = (
        f"Ты — профессиональный крипто- и фондовый трейдер с 10+ годами опыта. "
        f"Отвечай чётко, по делу, избегай фраз типа 'возможно', 'по-видимому', 'скорее всего'.\n\n"
        ...
        f"• Вопрос: {user_text}\n\n"
        f"Ответь по шагам:\n"
        f"1. Что важно учитывать в этом вопросе? Какие факторы ключевые?\n"
        f"2. Предложи основной сценарий действий.\n"
        f"3. Если он не сработает — что делать? Альтернативный сценарий.\n"
        f"4. Какой риск/потенциал выгоды в этом решении?\n"
        f"5. Если ты был бы на месте трейдера — что бы сделал прямо сейчас?\n\n"
        f"В конце добавь трейдерское резюме в 1 фразе: Вход / Не вход, с каким риском, зачем."
    )
...
async def handle_macro_for_image(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    prompt = (
        "Ты — профессиональный крипто-трейдер с опытом более 10 лет.\n"
...
async def handle_potential(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    prompt = (
        price_line +
        f"Ты — опытный криптоаналитик с 10+ годами опыта.\n"
...
async def handle_forecast_by_price(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    prompt = (
        price_line +
        f"Ты — профессиональный трейдер с опытом более 10 лет.\n"
...
async def publish_post(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("💰 Получить доступ", url="https://t.me/Parser_newbot")]
    ])
    ...
    text = (
        ...
        "💰 Всё это — за один раз: бот **навсегда** + курс = всего $25 (~3.4 TON)\n"
