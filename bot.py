...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "• Объясняет макро-новости и что с ними делать\n"
        "• Даёт VIP-сигналы: где вход, где стоп, где фиксировать\n"
...
async def handle_macro_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    prompt = (
        ...
        f"• Расставь ключевые факторы по степени важности (тренд, уровни, паттерны, объёмы).\n"
        f"• Укажи признаки накопления, разворота или импульса.\n\n"
        f"🌐 Учитывай фундаментальный фон:\n{macro}\n\n"
        f"🔁 Дай два сценария — при пробое вверх и при пробое вниз. Для каждого:\n"
        f"• Точки входа и стоп-лосс\n"
        f"• Потенциальная цель\n"
        f"• Вероятность и комментарий\n\n"
        f"Если видишь похожие паттерны на истории — укажи.\n"
        f"В конце посоветуй, какие данные ещё проверить для подтверждения сценария (например, объёмы, стакан, новости).\n\n"
        f"Отвечай подробно и исключительно на русском языке."
    )
...
async def publish_post(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    caption = (
        ...
        "• BlackRock, Fidelity, Grayscale скупают BTC и ETH на миллиарды через ETF\n"
        "• Московская биржа запускает криптофьючерсы для привлечения ликвидности\n"
        "• TON становится стандартом для крупных расчётов в СНГ: p2p, high-risk и маркетплейсы всё чаще выбирают именно его\n\n"
        "💰 Доходность на крипте по-прежнему кратно выше классических инструментов:\n"
        "• Недвижимость: ~8% годовых\n"
        "• Голубые фишки РФ: ~12–15%\n"
        "• BTC, ETH, TON — +30–100% за сделки с чётким риском и планом\n\n"
        "📈 Что даёт GPT-Трейдер:\n"
        "• Мгновенные прогнозы по скринам за 10 секунд\n"
        "• Разбор макро-новостей с готовыми сценариями\n"
        "• VIP-сигналы прямо в Telegram (с входами, стопами, фиксацией и уведомлениями о БУ)\n"
        "• GPT-психолог: юмор и поддержка после минусов, чтобы торговать без эмоциональных сливов\n\n"
        "🎁 *Подключившись сейчас, получаешь бонус:*\n"
        "📚 Полный курс по скальпингу и позиционке (10+ видео и PDF-гайды), даже если ты новичок.\n\n"
        "🚀 *Доступ навсегда всего за $25 (~3.4 TON)*.\n"
        "Зайди на рынок вместе с крупнейшими фондами и крупнейшими расчётами СНГ — но уже со своими правилами.\n\n"
        "👤 Подключись или задай вопрос 👉 [@zhbankov_alex](https://t.me/zhbankov_alex)"
    )
