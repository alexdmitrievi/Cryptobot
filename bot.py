...
async def gpt_psychologist_response(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_text = update.message.text.strip()
    prompt = (
        "Ты — GPT-психолог с чувством юмора, немного дерзкий, но добрый.\n"
        "Твоя специализация — поддержка трейдеров после потерь, неудач, фомо, лудомании и прочих бед.\n"
        "Говори не сухо, а с подколами, приколами, мемным вайбом. Используй эмодзи, чтобы сделать сообщение живым и тёплым.\n"
        "Ты как друг, который скажет правду в лицо, но приобнимет и даст дельный совет. Без токсичности, но с характером.\n\n"
...
async def start_therapy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "🧘‍♂️ Ну что, рынок опять дал под дых? Или просто душа просит поддержки?\n\n"
        "Давай, выгружай всё, что накипело — GPT-психолог на связи: выслушает, подколет, приобнимет и поможет вернуться в бой 😎💬",
        reply_markup=ReplyKeyboardRemove()
    )
    return WAITING_FOR_THERAPY_INPUT
