...
async def handle_potential(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    prompt = (
        f"Проанализируй криптовалюту {coin} как опытный криптоаналитик с 10+ годами опыта.\n"
        f"1. Перспективы роста монеты\n"
        f"2. Текущие тренды и настроение рынка\n"
        f"3. Основные уровни и зоны интереса\n"
        f"4. Фундаментальные и технические факторы\n"
        f"5. Есть ли смысл входить сейчас и с каким риском?\n\n"
        f"Ответ структурируй по пунктам, используй уверенный тон. Без фраз типа 'возможно'."
    )
    ...
    try:
        ...
        logging.error(f"[POTENTIAL] Ошибка анализа потенциала монеты: {e}")
        await update.message.reply_text("⚠️ Не удалось проанализировать монету. Попробуйте позже.")
